...
*** Thread shared types
Objects that are part of a function can be shared between threads. This is safe because they are marked as immutable, so they will not be edited. But we need some way to convince rust of this. Maybe we could make a ~Shared~ type that would let us mark things as immutable. Every object has some way to mark it immutable. This type would ensure that anything it holds has been marked immutable, and is therefore safe to share.
** Garbage collection
