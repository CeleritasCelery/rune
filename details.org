...
The fundamental problem is that the lisp object model and the rust object model. The model expects every allocation to have an 1 owner and that the allocation will be dropped when the object goes out of scope. The lisp model expects data to have many owners, and an allocation will be dropped when GC proves there are no more references to it. So my idea is to have two different object types. The underlying representation will be identical, but the way they are used will be different.
**** owned objects
These will not implement copy and also implement drop. This will make working in rust easier because we follow it's ownership semantics. But it also means we can't have multiple heads on a cons list and we can't control where a object is allocated.
**** referenced objects
These will implement copy but not drop. This means that we can build any structure we want, and control allocation. These will need to be GC'ed.
**** interop
When you are constructing an object to return to the lisp vm you will use the owned objects almost all the time. When we get objects from the VM they will be reference objects. I am still not sure if it would be better to make these a different object type, or just return a reference to the object. Probably should just start with a reference and then update as needed. So we may not even need a reference type defined, because a true reference already meets the bill. Only reason we would need something else would be for performance.
*** should we add lifetimes to the objects?
Since the lifetime of the actual data is tied to the arena, does it make sense to add lifetimes to all our objects? This would make "sure" that objects can't outlive their arena, and that objects returned from rust call aren't using the same arena. This would happen in two cases
- objects are put in a struct that lives globally
- the rust function creates a new arena and returns that.

Both of these seem like they could be avoided pretty easily. The other concern is when doing unit testing we want the arena to clean up after the test. But so long as the arena is declared at the top of the test and the test does not put objects in global structs.

The reason we may not want lifetimes is because they can be a real pain, and their is no good way to put lifetimed objects in struct (since this is a self-referencial struct, which rust hates). Which means we need to cheat everytime we have to put something in a stack or struct. Because at the end of the day we are not using lifetimes for memory managment, we are using garbage collection. The only thing we really need to avoid is objects getting collected that are still in use.

There is a third option, where we declare =lispobj= as =object<'static>=, which means that it will behave the extact same, but give us the option to limit the lifetimes if we wanted to.
*** tagged objects or enums
**** objects
- convert arrays in place
- have a function object for symbols
**** enum
- much cleaner code
- no need for .val function

only need to create into and tryinto traits to work with this.
