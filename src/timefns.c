...
tzlookup(Lisp_Object zone, bool settz)
{
  ...
  else
  {
    ...
    else if (plain_integer || (CONSP(zone) && FIXNUMP(XCAR(zone)) && CONSP(XCDR(zone))))
    {
    ...
    if (HAVE_TZALLOC_BUG && !new_tz && errno != ENOMEM && plain_integer && XFIXNUM(zone) % (60 * 60) == 0)
    {
...
enum
{
  flt_radix_power_size = DBL_MANT_DIG - DBL_MIN_EXP + 1
};
...
frac_to_double(Lisp_Object numerator, Lisp_Object denominator)
{
  ...
  if (FASTER_TIMEFNS && integer_to_intmax(numerator, &intmax_numerator) && integer_to_intmax(denominator, &intmax_denominator) && intmax_numerator % intmax_denominator == 0)
 ...
 so add FLT_RADIX to its absolute value as per round-to-even.  */
    ...
    incr = ((lo_digit > FLT_RADIX / 2 || (lo_digit == FLT_RADIX / 2 && FLT_RADIX % 2 == 0 && ((lo_2digits / FLT_RADIX) & 1 || mpz_sgn(*r) != 0)))
                ? FLT_RADIX
                : 0);
  }
...
timespec_ticks(struct timespec t)
{
  ...
  intmax_t accum;
  if (FASTER_TIMEFNS && !ckd_mul(&accum, t.tv_sec, TIMESPEC_HZ) && !ckd_add(&accum, accum, t.tv_nsec))
...
ticks_hz_seconds(struct ticks_hz t)
{
  ...
  if (FIXNUMP(t.ticks) && FIXNUMP(t.hz))
    return make_fixnum(XFIXNUM(t.ticks) / XFIXNUM(t.hz) - (XFIXNUM(t.ticks) % XFIXNUM(t.hz) < 0));
...
current_time_in_cform(enum cform cform)
{
  ...
  return ((FASTER_TIMEFNS && (cform == CFORM_TIMESPEC || cform == CFORM_SECS_ONLY))
              ? (union c_time){.ts = now}
  ...
  */
  ...
  else if (CONSP(specified_time))
  {
    ...
    if (CONSP(low))
    {
      ...
      struct err_time err_time = decode_time_components(high, low, usec, psec, hz, cform);
      if (err_time.err)
...
list4_to_timespec(Lisp_Object high, Lisp_Object low,
                  Lisp_Object usec, Lisp_Object psec)
{
  struct err_time err_time = decode_time_components(high, low, usec, psec, trillion, CFORM_TIMESPEC);
...
lispint_arith(Lisp_Object a, Lisp_Object b, bool subtract)
{
  ...
  if (FASTER_TIMEFNS && FIXNUMP(b))
  {
    ...
    {
      ((XFIXNUM(b) < 0) == subtract ? mpz_add_ui : mpz_sub_ui)(mpz[0], *xbignum_val(a), eabs(XFIXNUM(b)));
      mpz_done = true;
    }
  }
...
time_arith(Lisp_Object a, Lisp_Object b, bool subtract)
{
 ...
 skip this division if ihz = 1.  */
  ...
  return (BASE_EQ(hz, make_fixnum(1))
              ? ticks
          : (!current_time_list || (CONSP(a) && !CONSP(XCDR(a))) || (CONSP(b) && !CONSP(XCDR(b))) || !trillion_factor(hz))
              ? Fcons(ticks, hz)
...
(Lisp_Object specified_time, Lisp_Object zone, Lisp_Object form)
{
  ...
  Lisp_Object year;
  if (FASTER_TIMEFNS && MOST_NEGATIVE_FIXNUM - TM_YEAR_BASE <= local_tm.tm_year && local_tm.tm_year <= MOST_POSITIVE_FIXNUM - TM_YEAR_BASE)
  {
 ...
 and SEC = (TICKS . HZ).  */
    Lisp_Object ticks;
    intmax_t n;
    if (FASTER_TIMEFNS && FIXNUMP(th.ticks) && FIXNUMP(hz) && !ckd_mul(&n, XFIXNUM(hz), local_tm.tm_sec) && !ckd_add(&n, n, (XFIXNUM(th.ticks) % XFIXNUM(hz) + (XFIXNUM(th.ticks) % XFIXNUM(hz) < 0 ? XFIXNUM(hz) : 0))))
      ticks = make_int(n);
    else
    {
      mpz_fdiv_r(mpz[0],
...
int emacs_setenv_TZ(const char *tzstring)
{
