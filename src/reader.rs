...
impl<'a, 'ob> Reader<'a, 'ob> {
    ...
    fn read_sexp(&mut self, token: Token<'a>) -> Result<GcObj<'ob>> {
        match token {
            ...
            Token::Quote(i) => self.quote_item(i, SymbolX::new(&sym::QUOTE)),
            Token::Unquote(i) => self.quote_item(i, SymbolX::new(&sym::UNQUOTE)),
            Token::Splice(i) => self.quote_item(i, SymbolX::new(&sym::SPLICE)),
            Token::Backquote(i) => self.quote_item(i, SymbolX::new(&sym::BACKQUOTE)),
